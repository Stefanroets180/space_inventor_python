    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if stats.game_paused:
                ...
                # Touch/mouse control during gameplay - tap to shoot
                fire_bullet(ai_settings, screen, ship, bullets)
            else:
                check_play_button(
            ...
            # Touch/mouse control - move ship to follow mouse/touch position
            if stats.game_active:
                mouse_x, mouse_y = pygame.mouse.get_pos()
                ship.touch_control = True
                ship.target_x = mouse_x
        elif event.type == pygame.KEYDOWN:
            check_keydown_events(
                event, ai_settings, stats, sb, screen, ship, aliens, bullets)
            # Disable touch control when keyboard is used
            ship.touch_control = False
...
def start_game(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    # Keep mouse cursor visible for touch/mouse controls
...
def update_screen(
        ...
        bullets, alien_bullets, menu, fps_counter, fps, background):
    # Re-render screen with background image
